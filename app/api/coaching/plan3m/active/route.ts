...
const { data: active, error } = await supabase
  ...
  const activeId = active?.active_plan3m_id ?? null;
  if (!activeId) return jsonOk({ has_active_plan: false, active_plan3m_id: null, plan: null });
  ...
  return jsonOk({ has_active_plan: true, active_plan3m_id: activeId, plan: plan ?? null });
}
